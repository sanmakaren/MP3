...
All other factors being equal, the log odds of being approved is 1.90702, therefore probability of getting approved is approx 87%
...
submission <- test %>% 
  mutate(
    fitted_prob = 1/(1 + exp(-predict_no_pred$.fitted)), 
...
ggplot(data = fitted_points_model_2, aes(x=teacher_number_of_previously_posted_projects, y=project_is_approved)) +
  ...
  geom_line(data = fitted_points_no_pred, mapping = aes(y = fitted_prob), col = "blue", size = 1) + #plot model_1 probabilities
